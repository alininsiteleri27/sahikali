...
async function launchGame(gameType) {
    try {
        ...
        // Oyun tipine göre yönlendirme
        switch(gameType) {
            case 'chess':
                showToast('Satranç oyunu başlatılıyor...', 'info');
                // Satranç sayfasına yönlendir
                navigateToPage('games');
                break;
                
            case '2048':
                showToast('2048 oyunu başlatılıyor...', 'info');
                // 2048 oyunu başlat
                start2048Game();
                break;
                
            case 'wheel':
                showToast('Çarkıfelek oyunu başlatılıyor...', 'info');
                // Çarkıfelek sayfasına yönlendir
                navigateToPage('games');
                break;
                
            case 'rps':
                showToast('Taş-Kağıt-Makas oyunu başlatılıyor...', 'info');
                // Taş-Kağıt-Makas oyunu başlat
                startRPSGame();
                break;
                
            case 'coin':
                showToast('Yazı-Tura oyunu başlatılıyor...', 'info');
                // Yazı-Tura oyunu başlat
                startCoinGame();
                break;
                
            default:
                showToast('Oyun bulunamadı', 'error');
        }
...
function start2048Game() {
            ...
            <div class="modal-body">
                <div class="game-2048-container">
                    ...
                    <div class="game-instructions">
                        <p><strong>Kontroller:</strong> Ok tuşları veya WASD ile hareket edin</p>
                        <p>Aynı sayıları birleştirerek 2048 elde etmeye çalışın!</p>
                    </div>
                </div>
            </div>
        </div>
    ...
    // 2048 oyun mantığı BÖLÜM 2'de eklenecek
    showToast('2048 oyunu yükleniyor...', 'info');
}

// Taş-Kağıt-Makas Oyununu Başlat
function startRPSGame() {
    showToast('Taş-Kağıt-Makas oyunu yakında eklenecek', 'info');
    // BÖLÜM 3'te detaylı olarak eklenecek
}

// Yazı-Tura Oyununu Başlat
function startCoinGame() {
    showToast('Yazı-Tura oyunu yakında eklenecek', 'info');
    // BÖLÜM 3'te detaylı olarak eklenecek
}
...
class GameManager {
    ...
    async launchGame(gameType) {
        try {
            ...
            switch(gameType) {
                ...
                default:
                    showToast('Geçersiz oyun tipi', 'error');
                    return;
            }
    ...
    start2048Game() {
        // Modal zaten açıksa
        const existingModal = document.getElementById('game-2048-modal');
        if (existingModal) {
                ...
                <div class="modal-body">
                    <div class="game-2048-container">
                        <div class="game-2048-header">
                            ...
                            <div class="game-2048-best">
                                <span class="best-label">EN İYİ</span>
                                <span class="best-value" id="2048-best">0</span>
                            </div>
                        ...
                        <div class="game-2048-board" id="2048-board">
                            <div class="game-2048-grid" id="2048-grid"></div>
                        </div>
        ...
        modal.querySelector('.modal-close').addEventListener('click', () => {
            modal.remove();
            if (this.gameInstances.has('2048')) {
                this.gameInstances.get('2048').cleanup();
                this.gameInstances.delete('2048');
            }
        });
        ...
        game2048.loadBestScore();
        game2048.initialize();
        
        // Oyun instance'ını kaydet
        this.gameInstances.set('2048', game2048);
        this.currentGame = '2048';
    }
    
    // Satranç oyununu başlat
    startChessGame() {
        showToast('Satranç oyunu yakında eklenecek', 'info');
        // BÖLÜM 3'te detaylı olarak eklenecek
    }
    ...
    startRPSGame() {
                ...
                <div class="modal-body">
                    <div class="rps-container">
                        ...
                        <div class="rps-result" id="rps-result">
                            ...
                            <p>Yukarıdaki seçeneklerden birine tıklayın</p>
                        </div>
                        ...
                        <div class="game-stats">
                            <div class="stat-card">
                                ...
                                <div class="stat-info">
                                    <h3 class="stat-value" id="rps-wins">0</h3>
                            ...
                            <div class="stat-card">
                                ...
                                <div class="stat-info">
                                    <h3 class="stat-value" id="rps-losses">0</h3>
                                    <p class="stat-label">Mağlubiyet</p>
                                </div>
                            </div>
                            <div class="stat-card">
                                <div class="stat-icon">
                                ...
                                <div class="stat-info">
                                    <h3 class="stat-value" id="rps-draws">0</h3>
        ...
        // Seçim butonları
        modal.querySelectorAll('.rps-choice').forEach(choice => {
    ...
    async playRPS(playerChoice) {
        try {
            ...
            if (playerChoice === computerChoice) {
                ...
                // Puan ekle
                await this.addGamePoints('rps', 10);
            } else {
                result = 'KAYBETTİNİZ!';
                resultClass = 'lose';
            }
            ...
            resultElement.innerHTML = `
                ...
                <div class="rps-selections">
                    <div class="rps-selection">
                        <i class="${choiceIcons[playerChoice]}"></i>
                        <span>SEN<br>${choiceNames[playerChoice]}</span>
                    </div>
                    <div class="rps-vs">VS</div>
                    <div class="rps-selection">
                        <i class="${choiceIcons[computerChoice]}"></i>
                        <span>BİLGİSAYAR<br>${choiceNames[computerChoice]}</span>
                    </div>
                </div>
            ...
            // İstatistikleri güncelle
            await this.updateRPSStats(resultClass);
            
            // Tekrar oyna butonu
            document.getElementById('rps-play-again').addEventListener('click', () => {
                resultElement.innerHTML = `
                    <h2>Seçiminizi Yapın</h2>
                    <p>Yukarıdaki seçeneklerden birine tıklayın</p>
                `;
            ...
            console.error("Taş-Kağıt-Makas oynama hatası:", error);
            showToast('Oyun oynanırken bir hata oluştu', 'error');
        }
    }
    
    // Taş-Kağıt-Makas istatistiklerini yükle
    async loadRPSStats() {
        try {
            if (!AppState.currentUser) return;
            
            const statsDoc = await getDoc(doc(db, 'users', AppState.currentUser.uid, 'gameStats', 'rps'));
            
            if (statsDoc.exists()) {
                const stats = statsDoc.data();
                
                document.getElementById('rps-wins').textContent = stats.wins || 0;
                document.getElementById('rps-losses').textContent = stats.losses || 0;
                document.getElementById('rps-draws').textContent = stats.draws || 0;
            }
            
        } catch (error) {
            console.error("RPS istatistik yükleme hatası:", error);
        }
    }
    
    // Taş-Kağıt-Makas istatistiklerini güncelle
    async updateRPSStats(result) {
        try {
            if (!AppState.currentUser) return;
            
            const statsRef = doc(db, 'users', AppState.currentUser.uid, 'gameStats', 'rps');
            ...
            updates.played = increment(1);
            updates.lastPlayed = serverTimestamp();
            
            if (statsDoc.exists()) {
                await updateDoc(statsRef, updates);
            } else {
                updates.wins = updates.wins || 0;
                updates.losses = updates.losses || 0;
                updates.draws = updates.draws || 0;
                updates.played = updates.played || 1;
                await setDoc(statsRef, updates);
            }
            
            // UI'ı güncelle
            await this.loadRPSStats();
            
        } catch (error) {
            console.error("RPS istatistik güncelleme hatası:", error);
        }
    }
    ...
    startCoinGame() {
                ...
                <div class="modal-body">
                    <div class="coin-container">
                        <div class="coin-choices">
                            <div class="coin-choice" data-choice="heads">
                                <i class="fas fa-head-side-virus"></i>
                                <span>YAZI</span>
                            </div>
                            <div class="coin-choice" data-choice="tails">
                        ...
                        <div class="coin-result" id="coin-result">
                            <div class="coin-flip" id="coin-flip">
                                <i class="fas fa-coins"></i>
                            </div>
                            <h2>Tahmininizi Yapın</h2>
                            <p>Yazı mı tura mı geleceğini tahmin edin</p>
                        </div>
                        ...
                        <div class="game-stats">
                            <div class="stat-card">
                                ...
                                <div class="stat-info">
                                    <h3 class="stat-value" id="coin-wins">0</h3>
                                    <p class="stat-label">Doğru Tahmin</p>
                                </div>
                            </div>
                            <div class="stat-card">
                                <div class="stat-icon">
                                ...
                                <div class="stat-info">
                                    <h3 class="stat-value" id="coin-losses">0</h3>
                                    <p class="stat-label">Yanlış Tahmin</p>
                                </div>
                            </div>
                            <div class="stat-card">
                                <div class="stat-icon">
                                ...
                                <div class="stat-info">
                                    <h3 class="stat-value" id="coin-streak">0</h3>
                                    <p class="stat-label">Seri</p>
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        // Seçim butonları
        modal.querySelectorAll('.coin-choice').forEach(choice => {
            choice.addEventListener('click', (e) => {
                const playerChoice = e.currentTarget.getAttribute('data-choice');
                this.playCoinFlip(playerChoice);
            });
        });
    ...
    // Yazı-Tura oyna
    async playCoinFlip(playerChoice) {
        try {
            const coinFlip = document.getElementById('coin-flip');
            const resultElement = document.getElementById('coin-result');
            
            // Animasyon
            coinFlip.style.animation = 'spin 1s ease-in-out';
            
            // Rastgele sonuç
            const result = Math.random() < 0.5 ? 'heads' : 'tails';
            
            // 1 saniye sonra sonucu göster
            setTimeout(async () => {
                coinFlip.style.animation = '';
                
                const isWin = playerChoice === result;
                const resultText = isWin ? 'DOĞRU TAHMİN!' : 'YANLIŞ TAHMİN!';
                const resultClass = isWin ? 'win' : 'lose';
                const resultCoin = result === 'heads' ? 'YAZI' : 'TURA';
                ...
                resultElement.innerHTML = `
                    ...
                    <p>Sonuç: <strong>${resultCoin}</strong></p>
                    <p>Sizin tahmininiz: <strong>${playerChoice === 'heads' ? 'YAZI' : 'TURA'}</strong></p>
                    <button class="btn-primary" id="coin-flip-again">
                        <i class="fas fa-redo"></i>
                        Tekrar Dene
                    </button>
                `;
                ...
                // İstatistikleri güncelle
                await this.updateCoinStats(isWin);
                
                // Puan ekle
                if (isWin) {
                    await this.addGamePoints('coin', 5);
                }
                
                // Tekrar dene butonu
                document.getElementById('coin-flip-again').addEventListener('click', () => {
                    resultElement.innerHTML = `
                        <div class="coin-flip" id="coin-flip">
                            <i class="fas fa-coins"></i>
                        </div>
            ...
            console.error("Yazı-Tura oynama hatası:", error);
            showToast('Oyun oynanırken bir hata oluştu', 'error');
        }
    }
    
    // Yazı-Tura istatistiklerini yükle
    async loadCoinStats() {
        try {
            if (!AppState.currentUser) return;
            
            const statsDoc = await getDoc(doc(db, 'users', AppState.currentUser.uid, 'gameStats', 'coin'));
            
            if (statsDoc.exists()) {
                const stats = statsDoc.data();
                
                document.getElementById('coin-wins').textContent = stats.wins || 0;
                document.getElementById('coin-losses').textContent = stats.losses || 0;
                document.getElementById('coin-streak').textContent = stats.streak || 0;
            }
            
        } catch (error) {
            console.error("Yazı-Tura istatistik yükleme hatası:", error);
        }
    }
    
    // Yazı-Tura istatistiklerini güncelle
    async updateCoinStats(isWin) {
        try {
            if (!AppState.currentUser) return;
            
            const statsRef = doc(db, 'users', AppState.currentUser.uid, 'gameStats', 'coin');
            const statsDoc = await getDoc(statsRef);
            ...
            if (isWin) {
                ...
                updates.losses = increment(1);
                updates.streak = 0; // Seriyi sıfırla
            }
            ...
            if (statsDoc.exists()) {
                ...
                updates.wins = updates.wins || 0;
                updates.losses = updates.losses || 0;
                updates.streak = updates.streak || (isWin ? 1 : 0);
                updates.played = updates.played || 1;
                await setDoc(statsRef, updates);
            }
            ...
            console.error("Yazı-Tura istatistik güncelleme hatası:", error);
        }
    }
    
    // Çarkıfelek oyununu başlat
    startWheelGame() {
        showToast('Çarkıfelek oyunu yakında eklenecek', 'info');
        // BÖLÜM 3'te detaylı olarak eklenecek
    }
...
// Promise hatalarını yakala
window.addEventListener('unhandledrejection', function(e) {
    console.error('BÖLÜM 2 Promise hatası:', e.reason);
    showToast('Bir işlem sırasında hata oluştu', 'error');
});
